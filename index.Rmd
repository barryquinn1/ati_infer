...
subtitle: "Algorithms and statistical inference"
author: "Barry Quinn"
date: "`r Sys.Date()`"
output:
...
library(knitr)
knitr::opts_chunk$set(warning = FALSE, message = FALSE, fig.height=5, fig.width=7)
library(fontawesome) 
...
.pull-left[
- Suppose there’s a bag, and it contains four possible financial market future conditions. 
- These conditions can take two states: bull and bear. We know there are four conditions in the bag, but we don’t know how many are in each condition. 
- We do know that there are five possibilities in the bag:
...
- We do have some evidence: A sequence of three markets is pulled from the bag, one at a time, replacing the market each time and shaking the bag before drawing another market. 
- The sequence that emerges is: bull,bear,bull in that order. 
- These are the data.
]
...
#   scale_fill_manual(values  = c("navy", "bear")) +
...
.pull-left[
- The first draw turned out to be bull, If you imagine the real data tracing out a path through the garden, it must have passed through the one bull path near the origin. 
The second draw from the bag produces a bear market , so three of the paths forking out of the first bull market remain.
- Finally, the third draw is bull . 
- Visually we can see that by logically eliminated the other paths it leaves a total of three ways for the sequence to appear, assuming the bag contains [bull,bear,bear,bear].
]
.pull-right[
```{r echo=FALSE}
lines_1 <-
...
class:middle
# Conjecture summary
- To summarize, we’ve considered five different conjectures about the contents of the bag, ranging from zero bull markets to four bull markets. For each of these conjectures, we’ve counted up how many sequences, paths through the garden of forking data, could potentially produce the observed data,[bull,bear,bull]
...
t <-
  ...
  mutate(`draw 1: bull`  = n_bull(.),
         `draw 2: bear` = n_bear(.),
         `draw 3: bull`  = n_bull(.)) %>% 
  mutate(`ways to produce` = `draw 1: bull` * `draw 2: bear` * `draw 3: bull`)
...
.small[
- Luckily, there’s a mathematical way to compress all of this. 
- Specifically, *b=bull and w=bear* ,we define the updated plausibility of each possible composition of the bag, after seeing the data, as:
$$\text{plausibility of [bwww] after seeing [bwb]} \propto \text{ways[bwww] can produce [bwb]} \times \text{prior plausibility[bwww]} $$
- Probability can be thought of as plausibility standardise and if we helpfully define p=1/4 (the proportion of markets that are bull) and $D_{new}=[bwb]$:
...
- A conjectured proportion of bull markets, p, is usually called a .saltinline[parameter] value. It’s just a way of indexing possible explanations of the data.
...
- We the previous markets example we seen this.
...
## Comparison
.pull-left[
```{r compare,fig.cap="Bayesian inference proceeds vertically given x; frequentist inference proceeds horizontally, given mu", echo=FALSE}
library(ggrepel)
...
Quinn, Barry, Teaching Open Science Analytics in the Age of Financial Technology (January 27, 2022). QMS Research Paper 2022/01, Available at SSRN: https://ssrn.com/abstract=4019430 or http://dx.doi.org/10.2139/ssrn.4019430 - .hand[My Jerry Maguire Paper]
]
